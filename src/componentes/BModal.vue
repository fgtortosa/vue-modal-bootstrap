<template>
    <div v-if="modelValue" class="modal-backdrop fade show" role="presentation"></div>

    <div class="modal fade" :class="{
        'show': modelValue,
        'modal-static': props.required
    }" :style="{ display: modelValue ? 'block' : 'none' }" tabindex="-1" role="dialog" :id="props.modalId"
        :aria-modal="modelValue" :aria-hidden="!modelValue" @click.self="handleBackdropClick" data-bs-backdrop="static">
        <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 :id="headerId" class="modal-title h5">
                        <slot name="header"></slot>
                    </h2>
                    <button v-if="!props.required" type="button" class="btn-close" aria-label="Close"
                        @click="closeModal('close')"></button>
                </div>

                <div class="modal-body pb-0" :id="bodyId">
                    <slot name="body"></slot>
                </div>
